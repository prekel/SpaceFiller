...
type HydraReader(reader: System.Data.IDataReader) =
    ...
    let lazymainfill_record = lazy (main.fill_recordReader (reader, buildGetOrdinal 8))
    member __.``main.fill_record`` = lazymainfill_record.Value
    member private __.AccFieldCount with get () = accFieldCount and set (value) = accFieldCount <- value
    member private __.GetReaderByName(entity: string, isOption: bool) =
        match entity, isOption with
        | "main.fill_record", false -> __.``main.fill_record``.Read >> box
        | "main.fill_record", true -> __.``main.fill_record``.ReadIfNotNull >> box
